...
@Route(value = "myemployees", layout = MainView::class)
class MyEmployeeListView(
    ...
    private val employeeDialog = MyEmployeeFormDialog(
        myEmployeeService = myEmployeeService,
        onSave = { employee ->
            CoroutineScope(Dispatchers.IO).launch {
                myEmployeeService.save(employee)
                updateList()
            }
        },
        onDelete = { employee ->
            CoroutineScope(Dispatchers.IO).launch {
                myEmployeeService.delete(employee.id!!)
                updateList()
            }
        }
    )
    ...
    private fun updateList(filter: String? = null) {
        CoroutineScope(Dispatchers.IO).launch {
            val employees = myEmployeeService.findAll().filter { employee ->
